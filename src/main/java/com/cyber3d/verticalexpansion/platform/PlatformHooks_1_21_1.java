...
public final class PlatformHooks_1_21_1 implements PlatformHooks {
    ...
    @Override
    public void init() {
        LOGGER.info("Initializing VerticalExpansion for Minecraft 1.21.1");
        registerOreProfiles();
        initializeWorldGen();
        initializeFeatures();
    ...
    private void initializeWorldGen() {
        LOGGER.debug("Initializing world generation system for 1.21.1");
        WorldGenInitializer.initialize();
    }

    private void initializeFeatures() {
        LOGGER.debug("Initializing features for 1.21.1");
        FeatureRegistry.getInstance().initialize(WorldGenInitializer.getBiomePalette());
    }

    @Override
    public void registerChunkGenerator() {
        LOGGER.debug("Registering chunk generator for 1.21.1");
    }

    @Override
    public void registerBiomeModifiers() {
        LOGGER.debug("Registering biome modifiers for 1.21.1");
    }

    @Override
    public void hookBiomeLoadingEvents() {
        LOGGER.debug("Hooking biome loading events for 1.21.1");
    }

    @Override
    public void registerVerticalSectionIntegration() {
        LOGGER.debug("Registering vertical section integration for 1.21.1");
    }
}
